...
class PointEService:
    def __init__(self, device=None, quality='normal'):
        """
        Initialize Point-E service.
        
        Args:
            device: torch device ('cuda', 'cpu', or None for auto)
            quality: 'fast' (base40m only), 'normal' (base40m + upsample), 'high' (base64m + upsample)
        """
        # Determine device
    ...
    def _load_models(self):
        """Load Point-E models based on quality setting."""
        try:
            # Select base model based on quality
            # Available models: base40M, base40M-textvec, base40M-imagevec, base300M, base1B, upsample, sdf
            if self.quality == 'fast':
                self.base_name = 'base40M-textvec'
            elif self.quality == 'high':
                self.base_name = 'base40M-textvec' # base300M is image-only, use 40M for text
            else:  # normal
                self.base_name = 'base40M-textvec'
            ...
            # Load upsample model for non-fast quality
            if self.quality != 'fast':
